...
const ALL_TABS = [
  ...
  'Benefits', 'Notifications', 'Messages', 'Import Users', 'Admin Assign', 'Reg Questions', 'New Year', 'Card Mgmt'
];
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, users, benefits, notifications, messages = [], years, stats, onUpdateUser, onAddBenefit, onDeleteBenefit, onDeleteNotification, onSwitchToUserView, isLoading }) => {
...
Admin Team`);
  ...
  const handleReplyMessage = async () => {
      ...
      try {
          ...
          await StorageService.addNotification({
              ...
              title: `Reply: ${replyMessage.subject}`,
              message: replyContent,
              date: new Date().toLocaleDateString(),
              read: false,
              type: 'INDIVIDUAL',
              recipients: [replyMessage.userId],
              targetAudience: 'User'
          });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
                      ...
                      <tbody className="divide-y divide-slate-50">
                          {messages.map(m => (
                              <tr key={m.id} className="hover:bg-slate-50">
                                  <td className="px-6 py-4 text-xs text-slate-500 whitespace-nowrap">{m.date}</td>
                                  <td className="px-6 py-4">
                                  ...
                                  <td className="px-6 py-4 max-w-xs">
                                      ...
                                      {m.adminReply && (
                                          <p className="text-[10px] text-emerald-600 mt-1 bg-emerald-50 p-1 rounded w-fit">
                                              Replied: {m.adminReply}
                                          </p>
                                      )}
                                  </td>
                                  <td className="px-6 py-4">
                                      {m.status === 'REPLIED' ? (
                                          <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-full uppercase">Replied</span>
                                      ) : (
                                          <span className="px-2 py-1 bg-blue-100 text-blue-700 text-[10px] font-bold rounded-full uppercase">New</span>
                                      )}
                                  </td>
                                  <td className="px-6 py-4 text-right">
                                      <button 
                                          onClick={() => {
                                              setReplyMessage(m);
                                              setReplyContent(`Re: ${m.subject}\n\n`);
                                          }} 
                                          className="flex items-center gap-1 ml-auto text-xs bg-white border border-slate-200 hover:bg-slate-50 px-3 py-1.5 rounded-lg font-bold text-slate-700"
                                      >
                          ...
                          {messages.length === 0 && (
                              <tr><td colSpan={5} className="p-8 text-center text-slate-400">No messages found.</td></tr>
                          )}
                      </tbody>
                  </table>
              </div>
      ...
      {replyMessage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
              <div className="bg-white w-full max-w-lg rounded-xl shadow-xl p-6">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-lg">Reply to {replyMessage.userName}</h3>
                      <button onClick={() => setReplyMessage(null)}><X className="w-5 h-5 text-slate-400" /></button>
                  </div>
                  <div className="mb-4 bg-slate-50 p-3 rounded text-sm text-slate-600 italic border border-slate-100">
                      "{replyMessage.content}"
                  </div>
                  <textarea 
                      className="w-full p-3 border rounded-lg h-32 text-sm mb-4 outline-none focus:ring-2 focus:ring-primary/20"
                      placeholder="Type your reply here..."
                      value={replyContent}
                      onChange={e => setReplyContent(e.target.value)}
                  />
                  <div className="flex justify-end gap-2">
                      <button onClick={() => setReplyMessage(null)} className="px-4 py-2 bg-slate-100 rounded text-sm font-bold">Cancel</button>
                      <button onClick={handleReplyMessage} className="px-4 py-2 bg-primary text-white rounded text-sm font-bold flex items-center gap-2">
      ...
      {/* View User Modal (Restored Functionality for Eye Icon) */}
      {viewingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-6 bg-slate-50 border-b flex justify-between items-center">
                    <div className="flex items-center gap-4">
                         {viewingUser.photoUrl ? (
                             <img src={viewingUser.photoUrl} className="w-12 h-12 rounded-full object-cover border" />
                         ) : (
                         ...
                         <div>
                             ...
                             <p className="text-sm text-slate-500 font-mono">{viewingUser.membershipNo}</p>
                         </div>
                    </div>
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    {/* Credentials Section - Explicitly Requested by Admin */}
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="font-medium text-sm select-all">{viewingUser.email || viewingUser.mobile}</p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-400">Password</p>
                                <p className="font-mono font-bold text-sm bg-white border px-2 py-1 rounded w-fit select-all text-red-600 border-red-100">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                        ...
                        <div>
                            ...
                            <p className="font-medium">{viewingUser.whatsapp}</p>
                        </div>
                        <div>
                            <p className="text-xs text-slate-400">Emirates ID</p>
                            <p className="font-medium">{viewingUser.emiratesId}</p>
                        </div>
                        <div>
                            <p className="text-xs text-slate-400">Status</p>
                            <span className={`px-2 py-0.5 rounded text-xs font-bold ${viewingUser.status === 'APPROVED' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                                {viewingUser.status}
                            </span>
                        </div>
                        <div>
                            <p className="text-xs text-slate-400">Payment</p>
                            <span className={`px-2 py-0.5 rounded text-xs font-bold ${viewingUser.paymentStatus === 'PAID' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>
                                {viewingUser.paymentStatus}
                            </span>
                        </div>
                        <div>
                            <p className="text-xs text-slate-400">Mandalam</p>
                            <p className="font-medium">{viewingUser.mandalam}</p>
                        </div>
                    </div>

                    {/* Custom Fields */}
                    {viewingUser.customData && (
                        <div className="border-t pt-4">
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">Additional Details</h4>
                            <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                                {Object.entries(viewingUser.customData).map(([key, val]) => (
                                    <div key={key}>
                                        <p className="text-xs text-slate-400">{questions.find(q=>q.id===key)?.label || key}</p>
                                        <p className="font-medium text-slate-700 break-words">{val as string}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
