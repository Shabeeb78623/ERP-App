  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {activeTab === 'Notifications' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4"><h3 className="font-bold text-slate-800">Send Notification</h3><input className="w-full p-2 border rounded" placeholder="Title" value={notifTitle} onChange={e=>setNotifTitle(e.target.value)} /><textarea className="w-full p-2 border rounded h-24" placeholder="Message..." value={notifMessage} onChange={e=>setNotifMessage(e.target.value)} /><select className="w-full p-2 border rounded" value={notifTarget} onChange={e=>setNotifTarget(e.target.value)}><option value="ALL">All Members</option>{MANDALAMS.map(m => <option key={m} value={m}>{m} Members</option>)}</select><button onClick={handleSendNotification} disabled={sendingNotif} className="w-full py-2 bg-primary text-white rounded font-bold hover:bg-primary-dark">{sendingNotif?'Sending...':'Send'}</button></div>
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm overflow-y-auto max-h-[500px]"><h3 className="font-bold text-slate-800 mb-4">History</h3><div className="space-y-4">{notifications.map(n => (<div key={n.id} className="p-3 border rounded hover:bg-slate-50 flex justify-between"><div><p className="font-bold text-sm">{n.title}</p><p className="text-xs text-slate-500">{n.date} • {n.targetAudience}</p></div><button onClick={()=>onDeleteNotification(n.id)} className="text-red-400"><Trash2 className="w-4 h-4"/></button></div>))}</div></div>
          </div>
      )}
